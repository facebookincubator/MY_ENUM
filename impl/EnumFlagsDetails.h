...
// Output: , ${FLAG} = (1 << ($I + 1))
#define MY_ENUMFLAGS_DETAILS_OP_INIT_FLAG(dummy1, dummy2, I, FLAG) , FLAG = (1 << (I + 1))
...
#define MY_ENUMFLAGS_DETAILS_FLAG_DEFINITIONS(...)                 \
  BOOST_PP_TUPLE_ELEM(0, __VA_ARGS__) = 1 BOOST_PP_SEQ_FOR_EACH_I( \
      MY_ENUMFLAGS_DETAILS_OP_INIT_FLAG,                           \
      _,                                                           \
...
// Output: ", ${STRING}"
#define MY_ENUMFLAGS_DETAILS_OP_COMMA_STRING(dummy1, dummy2, STRING) , BOOST_PP_STRINGIZE(STRING)
...
#define MY_ENUMFLAGS_DETAILS_COMMA_SEP_STRINGS(...)       \
  ...
  BOOST_PP_SEQ_FOR_EACH(                                  \
      MY_ENUMFLAGS_DETAILS_OP_COMMA_STRING,               \
      _,                                                  \
      BOOST_PP_SEQ_POP_FRONT(BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__)))
...
#define MY_ENUMFLAGS_DETAILS_CSV_STRING(...)              \
  BOOST_PP_STRINGIZE(BOOST_PP_TUPLE_ELEM(0, __VA_ARGS__)) \
  BOOST_PP_SEQ_FOR_EACH(                                  \
      MY_ENUMFLAGS_DETAILS_OP_COMMA_VALUE_STRING,         \
      _,                                                  \
      BOOST_PP_SEQ_POP_FRONT(BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__)))

// Output: , uint64_t(${TYPE}::${VAL})
#define MY_ENUMFLAGS_DETAILS_OP_COMMA_INT(dummy1, TYPE, VAL) , uint64_t(TYPE::VAL)
...
#define MY_ENUMFLAGS_DETAILS_COMMA_SEP_INTS(TYPE, ...)                       \
  uint64_t(TYPE::BOOST_PP_TUPLE_ELEM(0, __VA_ARGS__)) BOOST_PP_SEQ_FOR_EACH( \
      MY_ENUMFLAGS_DETAILS_OP_COMMA_INT,                                     \
      TYPE,                                                                  \
      BOOST_PP_SEQ_POP_FRONT(BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__)))
...
#define MY_ENUMFLAGS_DEF_IMPL(NAME, UINT_TYPE, ...)                                              \
  ...
  static_assert(                                                                                 \
      ...
      BOOST_PP_STRINGIZE(UINT_TYPE) " must be an unsigned integer type");                        \
                                                                                                 \
  enum class NAME##Impl : UINT_TYPE{none = 0u,                                                   \
                                    MY_ENUMFLAGS_DETAILS_FLAG_DEFINITIONS(__VA_ARGS__)};         \
                                                                                                 \
  ...
  MY_ENUM_MAYBE_UNUSED inline bool trySetFlagFromString(                                         \
    ...
    BOOST_PP_SEQ_FOR_EACH(                                                                       \
        MY_ENUMFLAG_DETAILS_OP_SET_ENUM_FROM_STRING,                                             \
        NAME##Impl,                                                                              \
        BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__))                                                      \
    return false;                                                                                \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED inline bool hasMask(NAME##Impl value, NAME##Impl mask) {                  \
    return (value & mask) == mask;                                                               \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED inline void setMask(NAME##Impl& value, NAME##Impl mask) {                 \
    value = value | mask;                                                                        \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED inline void clearMask(NAME##Impl& value, NAME##Impl mask) {               \
    value = value & NAME##Impl(~UINT_TYPE(mask));                                                \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED inline void toggleMask(NAME##Impl& value, NAME##Impl mask) {              \
  ...
  MY_ENUM_MAYBE_UNUSED inline bool isSingleFlag(NAME##Impl value) {                              \
    switch (value) {                                                                             \
      BOOST_PP_SEQ_FOR_EACH(                                                                     \
          MY_ENUMFLAG_DETAILS_OP_SINGLE_FLAG_CHECK,                                              \
          NAME##Impl,                                                                            \
          BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__))                                                    \
      case NAME##Impl::none:                                                                     \
        break;                                                                                   \
    ...
    return false;                                                                                \
                                                                                                 ...
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED inline std::vector<std::string> toStrings(NAME##Impl value) {             \
    std::vector<std::string> strings;                                                            \
    BOOST_PP_SEQ_FOR_EACH(                                                                       \
        MY_ENUMFLAG_DETAILS_OP_PUSHBACK_STRING, NAME##Impl, BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__))  \
    return strings;                                                                              \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED inline std::string toPretty(NAME##Impl value) {                           \
    ...
    BOOST_PP_SEQ_FOR_EACH(                                                                       \
        MY_ENUMFLAG_DETAILS_OP_STRING_FOR_FLAG, NAME##Impl, BOOST_PP_TUPLE_TO_SEQ(__VA_ARGS__))  \
    prettySet += "} (=" + std::to_string(UINT_TYPE(value)) + ")";                                \
    return prettySet;                                                                            \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED MY_ENUM_NODISCARD constexpr size_t getCount(NAME##Impl) {                 \
    return BOOST_PP_TUPLE_SIZE(__VA_ARGS__);                                                     \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED                                                                           \
  MY_ENUM_NODISCARD inline std::array<fmt::string_view, BOOST_PP_TUPLE_SIZE(__VA_ARGS__)>        \
  getNames(NAME##Impl) {                                                                         \
    return {MY_ENUMFLAGS_DETAILS_COMMA_SEP_STRINGS(__VA_ARGS__)};                                \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED MY_ENUM_NODISCARD inline fmt::string_view getStringOfNames(NAME##Impl) {  \
    return MY_ENUMFLAGS_DETAILS_CSV_STRING(__VA_ARGS__);                                         \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED                                                                           \
  MY_ENUM_NODISCARD constexpr std::array<UINT_TYPE, BOOST_PP_TUPLE_SIZE(__VA_ARGS__)> getValues( \
      NAME##Impl) {                                                                              \
    return {MY_ENUMFLAGS_DETAILS_COMMA_SEP_INTS(NAME##Impl, __VA_ARGS__)};                       \
  }                                                                                              \
                                                                                                 \
  MY_ENUM_MAYBE_UNUSED MY_ENUM_NODISCARD inline fmt::string_view getTypeName(NAME##Impl) {       \
